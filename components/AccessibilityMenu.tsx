  ...
  return (
    <>
      ...
      >
        ...
        <button 
          className={`
            bg-[#2c1810] text-[#faf6ee] p-4 rounded-full shadow-2xl transition-transform duration-150
        ...
        {isOpen && (
          <div 
            className={`
              a11y-panel absolute w-80 p-6 bg-white border-2 border-[#2c1810] rounded-2xl shadow-2xl flex flex-col gap-6 cursor-default
              animate-in slide-in-from-bottom-5
              ${openUpwards ? "bottom-[calc(100%+16px)]" : "top-[calc(100%+16px)]"}
              ${openLeftwards ? "right-0" : "left-0"}
            `}
            onPointerDown={(e) => e.stopPropagation()} // Prevents dragging when clicking settings
          >
            <div className="flex items-center justify-between border-b pb-3">
              <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                <Accessibility size={20} /> Accessibility
              </h2>
              <button onClick={() => setIsOpen(false)} className="text-gray-500 hover:text-red-500 font-bold text-xl leading-none">×</button>
            </div>
