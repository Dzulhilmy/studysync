            ...
            <div className="px-5 py-3 space-y-1.5 border-b border-[#f0e9d6]">
              <div className="flex justify-between text-xs font-mono">
                ...
                <span className={popupUser.isActive ? "text-[#1a7a6e] font-bold" : "text-[#c0392b]"}>
                  {popupUser.isActive ? "● Active" : "○ Inactive"}
                </span>
              </div>
              <div className="flex justify-between text-xs font-mono">
                <span className="text-[#a89880]">Joined</span>
                <span className="text-[#4a3828]">
                  {new Date(popupUser.createdAt).toLocaleDateString("en-MY", { day: "numeric", month: "short", year: "numeric" })}
                </span>
              </div>
              {popupUser.lastLogin && (
                <div className="flex justify-between text-xs font-mono">
                  <span className="text-[#a89880]">Last login</span>
                  <span className="text-[#4a3828]">
                    {new Date(popupUser.lastLogin).toLocaleDateString("en-MY", { day: "numeric", month: "short" })}
                  </span>
                </div>
              )}
            </div>
