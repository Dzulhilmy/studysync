...
function BreakdownModal({
  type, reportId, onClose,
}: {
  type: BreakdownType;
  reportId: string;
  onClose: () => void;
}) {
  const [students, setStudents] = useState<BreakdownStudent[]>([]);
  ...
  useEffect(() => {
    fetch(`/api/admin/reports/${reportId}/breakdown?type=${type}`)
      .then(r => r.json())
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white border border-[#c8b89a] rounded-sm shadow-[5px_5px_0_#c8b89a] w-full max-w-lg max-h-[80vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-[#f0e9d6]">
          {loading ? (
            <div className="p-10 text-center text-[#7a6a52] text-sm font-mono animate-pulse">Loading students…</div>
          ) : students.length === 0 ? (
            ...
            <div key={i} className="px-5 py-3 grid grid-cols-12 gap-2 items-center hover:bg-[#faf6ee]">
              ...
              <div className="col-span-4 text-xs text-[#4a3828] truncate">{s.projectTitle ?? "—"}</div>
...
export default function AdminReportsPage() {
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        <p className="text-[#c0392b] text-xs font-mono tracking-[0.2em] uppercase mb-1">レポート管理</p>
          ...
          <div className="lg:col-span-3">
            {!selected ? (
              ...
              <div className="space-y-4">
                ...
                {/* ── Submission breakdown — clickable cards open student list ── */}
                <div className="bg-white border border-[#c8b89a] rounded-sm p-4 shadow-[2px_2px_0_#c8b89a]">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-[10px] font-mono text-[#a89880] italic">Click a card to see who</span>
                  </div>
                  {(() => {
